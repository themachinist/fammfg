		   ...
		   <button class="btn btn-default dropdown-toggle" data-toggle="dropdown">@lang('button.actions')
			...
			<ul class="dropdown-menu">
				@if ($model->deleted_at=='')
					...
					<li><a href="{{ route('create/assets', $model->id) }}">@lang('admin/assets/form.create')</a></li>
				@else
...
<div class="user-profile">
			...
			@else
				<div class="col-md-9">
					<div class="alert alert-info alert-block">
						<i class="fa fa-info-circle"></i> @lang('general.no_results')
					</div>
				</div>
			@endif
	...
	<h6>More Info:</h6>
		...
		@if ($model->manufacturer)
			<li>@lang('general.manufacturer'): {{ $model->manufacturer->name }}</li>
		@endif

		@if ($model->modelno)
			<li>@lang('general.model_no'): {{ $model->modelno }}</li>
		@endif
