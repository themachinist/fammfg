...
<form class="form-horizontal" method="post" action="" autocomplete="off">
    ...
    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
            ...
            <label class="col-sm-3 control-label">@lang('admin/tools/general.tool_name')</label>
                <div class="col-md-6">
                  <p class="form-control-static">{{{ $tool->name }}}</p>
                </div>
            </div>
            @endif
            ...
            <!-- tool name -->
            <div class="form-group">
            <label class="col-sm-3 control-label">@lang('admin/tools/general.tool_category')</label>
                <div class="col-md-6">
                  <p class="form-control-static">{{{ $tool->category->name }}}</p>
                </div>
            </div>
            @endif
            ...
            <div class="form-group {{ $errors->has('assigned_to') ? ' has-error' : '' }}">
                ...
                <div class="col-md-9">
                    {{ Form::select('assigned_to', $users_list , Input::old('assigned_to', $tool->assigned_to), array('class'=>'select2', 'style'=>'min-width:350px')) }}
                    {{ $errors->first('assigned_to', '<br><span class="alert-msg"><i class="fa fa-times"></i> :message</span>') }}
                </div>
            </div>

			@if ($tool->category->require_acceptance=='1')
			<div class="form-group">
